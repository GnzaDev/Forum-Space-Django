...
def ver_post(request, post_id):
    ...
    respuestas = post.respuesta_set.order_by('fecha_creacion')
...
def eliminar_post(request, post_id):
    ...
    # Permitir eliminar si es el autor o si es admin/staff
    if post.autor != request.user and not (request.user.is_staff or request.user.is_superuser):
    ...
    if request.method == 'POST':
        ...
        # Redirigir al panel de admin si es admin, sino al índice
...
def admin_panel(request):
    posts_pendientes = Post.objects.all().distinct()
